...
// PromptDisplay component for expanding/collapsing long prompts
  ...
  useEffect(() => {
    const handleInitialResponse = async () => {
      ...
      if (activeModels.length > 0) {
        ...
        // console.log(modelId, 'here is the model id');
        // console.log(promptId, 'here is the prompt id');
        // console.log(conversationId, 'here is the conversation id');
        try {
    ...
    const loadConversation = async () => {
      ...
      setActiveTab('ag'); // Set active tab to 'tts' when loading conversation
      ...
      try {
        ...
        if (response && response[0]) {
          ...
          if (audioResponse) {
            ...
            setModelResult(result);
            setCategoryModel('ag', audioResponse.model.uid);
            // After saving, set the selected model for the current tab from the store
            const storeModelId = getCategoryModel('ag');
            if (storeModelId) {
    ...
    <div className={`container max-w-5xl mx-auto py-4 px-4 flex flex-col h-full ${isOpen ? "pl-40" : "pl-6"}`}>
      ...
      {initialPrompt && (
        <div className="flex items-start justify-between mb-10">
          <div className="text-base break-words pr-4">
            <PromptDisplay text={initialPrompt} maxLength={100} />
          </div>
          <button
            onClick={() => {
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><rect x="9" y="9" width="13" height="13" rx="2" strokeWidth="2" stroke="currentColor"/><rect x="3" y="3" width="13" height="13" rx="2" strokeWidth="2" stroke="currentColor"/></svg>
          </button>
        </div>
      )}
      {/* End prompt display */}
